...
class ConsoleClientCliParser {
    ...
    private final IntCliParameter userId = new CliParameterBuilder("-uid").setDescription(
            "The user ID which identifies the user. The user ID is carried by the calling-AE-invocation-id field of the AARQ.")
            .buildIntParameter("user_ID");
    ...
    public void parse(String[] args) throws CliParseException {
        ...
        if (authenticationMechanism.getValue() == 1 && !password.isSelected()) {
    ...
    private GenActionProcessor setCommonParameters(ConnectionBuilder<?> connectionBuilder)
        ...
        connectionBuilder.setLogicalDeviceId(logicalDeviceAddress.getValue())
                ...
                .setUserId(userId.isSelected() ? userId.getValue() : -1)
