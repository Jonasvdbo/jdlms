...
abstract class BaseDlmsConnection implements BaseConnection {
    ...
    private BaseDlmsConnection() {
        sessionLayer = null;
        incomingApdQueue = null;
        settings = null;
        cosemResponseQ = null;
    }

    public static BaseDlmsConnection getTestInstance() {
        return new BaseDlmsConnection() {
            @Override
    ...
    public synchronized void disconnect() throws IOException {
        try {
            ...
            APdu aPdu = null;
            try {
                aPdu = this.incomingApdQueue.poll(settings.responseTimeout(), TimeUnit.MILLISECONDS);
                if (aPdu == null) {
    ...
    void connect() throws IOException {
        ...
        AEInvocationIdentifier aeInvocationIdentifier = aeInvocationIdentifier(settings.userId());
    ...
    private AEInvocationIdentifier aeInvocationIdentifier(int userId) {
        if (userId < 0) {
            ...
            return new AEInvocationIdentifier(settings.userId());
        }
    }
    ...
    private COSEMpdu pollValidResponsePdu() throws IOException {
        COSEMpdu responsePdu = this.cosemResponseQ.poll(settings.responseTimeout(), TimeUnit.MILLISECONDS);
        ...
        if (settings.referencingMethod() == ReferencingMethod.LOGICAL) {
            for (int i = 0; i < 3; i++) {
                ...
                responsePdu = this.cosemResponseQ.poll(settings.responseTimeout(), TimeUnit.MILLISECONDS);
            }
        }
    ...
    private APdu waitForServerResponseAPdu() throws IOException {
        try {
            return incomingApdQueue.poll(settings.responseTimeout(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            // Restore interrupted state...
            Thread.currentThread().interrupt();
