...
public class GcmModule {
    ...
    public static byte[] processPlain(byte[] plaintext, int off, int len, byte[] systemTitle, long frameCounter,
        ...
        byte[] cipherText = new byte[encCipher.getOutputSize(len)];
        int length = encCipher.processBytes(plaintext, off, len, cipherText, 0);
        try {
            encCipher.doFinal(cipherText, length);
        ...
        byte[] lengthBytes = AxdrLength.encodeLength(cipherTextLength + 5);

        final int cipheredApduLength = cipherTextLength + LENGTH_FC_TAG_SC + lengthBytes.length;
        ...
        return ByteBuffer.allocate(cipheredApduLength)
                ...
                .put(cipherText, 0, cipherTextLength)
                .array();
    }
    ...
    public static byte[] decrypt(byte[] cipheredApdu, byte[] systemTitle, SecuritySuite securitySuite)
        ...
        byte[] iv;
        try {
            iv = ByteBuffer.allocate(IV_LENGTH).put(systemTitle).put(cipheredApdu, 1, FC_LENGTH).array();
        } catch (BufferOverflowException e) {
            throw new DecryptionException("Buffer overflow.");
        } catch (IndexOutOfBoundsException e) {
            throw new DecryptionException("Index out of bounds.");
        } catch (RuntimeException e) {
            throw new DecryptionException("Unknown error.");
        }
