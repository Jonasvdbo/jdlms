...
public class CosemDateTimeTest {
    ...
    @ParameterizedTest
    @CsvSource({ "2016, 13, 13", "2016, 12, 32", "99, 2, 0", "2, 66, 0", "2, 59, 77" })
    @DisplayName("test error handling of codem date withe year = {0}, month = {1}, day = {2}")
    public void testDateExCase1(final int year, final int month, final int day) throws Exception {
    ...
    public void decodingTest() throws IOException {
        ...
        InputStream inputStream = new InputStream() {
            byte[] values = new byte[] { 1, 66, 0, 25, 7, -27, 5, 14, 5, 10, 32, 30, 0, -1, -120, -128 };
            public int pos = 0;
        ...
        assertEquals(16, getResponse.decode(inputStream));
        CosemDateTime dateTime = CosemDateTime.decode(getResponse.getResponseNormal.result.data.dateTime.getValue());
        assertEquals(2021, dateTime.get(Field.YEAR));
        assertEquals(5, dateTime.get(Field.MONTH));
        assertEquals(14, dateTime.get(Field.DAY_OF_MONTH));
        assertEquals(5, dateTime.get(Field.DAY_OF_WEEK));
        assertEquals(10, dateTime.get(Field.HOUR));
        assertEquals(32, dateTime.get(Field.MINUTE));
        assertEquals(30, dateTime.get(Field.SECOND));
        assertEquals(0, dateTime.get(Field.HUNDREDTHS));
        assertEquals(-120, dateTime.get(Field.DEVIATION));
        assertEquals(-128, dateTime.get(Field.CLOCK_STATUS));
    }

    @Test
    public void decodingTest2() throws IOException {
        GetResponse getResponse = new GetResponse();
        InputStream inputStream = new InputStream() {
            byte[] values = new byte[] { 1, 66, 0, 26, 7, -27, 5, 14, 5 };
            public int pos = 0;
        ...
        assertEquals(9, getResponse.decode(inputStream));
        CosemDate date = CosemDate.decode(getResponse.getResponseNormal.result.data.date.getValue());
        assertEquals(2021, date.get(Field.YEAR));
        assertEquals(5, date.get(Field.MONTH));
        assertEquals(14, date.get(Field.DAY_OF_MONTH));
        assertEquals(5, date.get(Field.DAY_OF_WEEK));
    }

    @Test
    public void decodingTest3() throws IOException {
        GetResponse getResponse = new GetResponse();
        InputStream inputStream = new InputStream() {
            byte[] values = new byte[] { 1, 66, 0, 27, 10, 32, 30, 0, -1, -120, -128 };
            public int pos = 0;
        ...
        CosemTime time = CosemTime.decode(getResponse.getResponseNormal.result.data.time.getValue());
        assertEquals(10, time.get(Field.HOUR));
        assertEquals(32, time.get(Field.MINUTE));
        assertEquals(30, time.get(Field.SECOND));
        assertEquals(0, time.get(Field.HUNDREDTHS));
    }
}
